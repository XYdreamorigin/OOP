...
void player::upgrade(vector<player>::iterator it,int play_level)  //升级，参数是vector<player>的原因是因为当前玩家cur_player是该类型的
{																  //第二个参数是玩家玩的难度
...
void game::start_player_game()
{
	...
	while (play_choice != 0)
	{
		...
		//总的来说这个循环就是找到一个单词表的范围，根据难度选择单词表的范围，1-4个字母的单词为第一级别，4-8,9-12,12-16,共有4个
		for (auto search_size : all_words)  //查找该长度的第一个单词
		{
			loc++;
			if (search_size.length() > (passed_level-1)*4)   
			{
				words_start = loc;
				loc = 0;
				break;
		...
		{
			loc++;
			if (passed_level == all_level) //如果当前难度最大
			{
				words_end = all_words.size(); //第五难度的末尾就是词库末尾
				break;
			}
			if (search_size.length() > passed_level * 4)
			{
				words_end = loc - 1;
				loc = 0;
				break;
		...
		if (input_word == all_words.at(loc))//正确
		{
			...
			/*更新等级、经验*/
			player::upgrade(cur_player,passed_level); //当前玩家加经验,闯过关卡增加,根据难度加经验
		}
		else//错误
		{
